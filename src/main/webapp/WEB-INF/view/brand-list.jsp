...
<script type="text/javascript">
	        ...
	        fields: {
	        	brandName: {
	                ...
	                validators: {
	                    ...
	                    notEmpty: {
	                    	//不符合规定的提示语句
	                        message: '<font color = "red" size = "2">*品牌名不能为空</font>'
	                    }
	                }
	            }
	        }
	...
	function deleteBrand(id){
		...
		location.href = "<%=request.getContextPath()%>/brand/deleteBrand?id="+id;
	...
	function toUpdateBrand(id){
		$.ajax({
			url:"<%=request.getContextPath()%>/brand/toUpdateBrand",
			data:{id:id},
			type:"post",
			dataType:"html",
	...
	function getData(){
		...
		var param = {"cpage":cpage,"pageSize":pageSize,"brandName":brandName,"beginDate":beginDate,"endDate":endDate};
		$.ajax({
			url:"<%=request.getContextPath()%>/brand/pagingQueryBrand",
			dataType:"json",
			type:"post",
			data:param,
			success:function(res){
				...
				var columName  = [
						              ...
						              {"data":function(data){ 	//data是你的数据集合   通过你的属性名 可以直接.出来 这个data 只是个名字
						              	return '<button class="btn btn-danger btn-sm" onclick = "deleteBrand('+"'"+data.id+"'"+')"><span class="glyphicon glyphicon-trash" style="color: #ffffff;"></span> '+
											   ' </button><button class="btn btn-info btn-sm" onclick = "toUpdateBrand('+"'"+data.id+"'"+')" data-toggle="modal" data-target="#toUpdate"><span class="glyphicon glyphicon-pencil" style="color: #ffffff;"></span></button>';
				              	 }}];
				/* 定义一个json数组 放表头所需要的数据 给表头的tr赋值   */
				var tabHead  = [{"data":"选择"},{"data":"品牌名称"},{"data":"创建时间"},{"data":"操作"}];
...
<body background="<%=request.getContextPath()%>/img/3.jpg">
	...
	<div><!-- 上 左 右 -->
		<div class="input-group">
			...
			<input type="text" id="brandName" class="form-control" placeholder="请输入品牌名称" style="width: 200px"/><br/>
	...
	<input type="button" value = "Search" onclick="getData()" style = "width: 100px;margin-left: 400px;">
	
	<!-- 新增 -->
	<div id="save" class="modal fade" tabindex="-1">
        <h1 style = "color: white;">Insert Brand</h1>
        
        <div class="sub-main-w3" class="form-group">
            <form action="<%=request.getContextPath()%>/brand/saveBrand" method = "post" id = "defaultForm">
            	<h2 style = "color: white;">Insert Now</h2>
                <div class="form-style-agile">
                    <label class="col-lg-3 control-label">BrandName</label>
                    <div class="col-lg-5">
                    	<input type="text" placeholder="BrandName" name = "brandName" style = "width: 350px;"/>
                    </div>
                </div>
